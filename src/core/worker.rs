...
impl WorkerFactory {
    ...
    pub fn create_workers(urls: Vec<String>, data_parallel_size: usize) -> Vec<Box<dyn Worker>> {
        if data_parallel_size > 1 {
            // Create DP-aware workers: one worker per URL per DP rank
            urls.into_iter()
                .flat_map(|url| {
                    (0..data_parallel_size).map(move |rank| {
...
mod tests {
    ...
    fn test_factory_create_workers_with_dp() {
        ...
        for (i, worker) in workers.iter().enumerate().take(4) {
            assert!(worker.is_dp_aware());
            assert_eq!(worker.base_url(), "http://w1:8080");
            assert_eq!(worker.dp_rank(), Some(i));
            assert_eq!(worker.dp_size(), Some(4));
        }

        // Next 4 workers should be for w2:8080
        for (idx, worker) in workers.iter().enumerate().skip(4).take(4) {
            assert!(worker.is_dp_aware());
            assert_eq!(worker.base_url(), "http://w2:8080");
            assert_eq!(worker.dp_rank(), Some(idx - 4));
            assert_eq!(worker.dp_size(), Some(4));
        }
    }
